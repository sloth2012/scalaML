...
import com.lx.algos.newml.loss.{LossFunction, SoftmaxLoss}
...
class LogisticRegression extends Estimator with WeightMatrix {
  ...
  private val lossf: LossFunction = new SoftmaxLoss


  //TODO logger
  def logger(epoch: Int, acc: Double, avg_loss: Double): Unit = {
  ...
  override def fit(X: DenseMatrix[Double], y: DenseMatrix[Double]): Estimator = {
    assert(X.rows == y.rows)

    var (data, label) = if (fit_intercept) (DenseMatrix.horzcat(DenseMatrix.ones[Double](X.rows, 1), X), y)
    else (X, y)
    weight_init(data.rows, data.cols)
...
//            val acc = ClassificationMetrics.accuracy_score()
          }
        }
      }
    }
  ...
  override def predict(X: DenseMatrix[Double]): DenseMatrix[Double] = lossf.value(_theta, X)

  override def predict_proba(X: DenseMatrix[Double]): DenseMatrix[Double] = ???
}
