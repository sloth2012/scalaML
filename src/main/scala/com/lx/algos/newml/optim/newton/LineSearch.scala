...
class WolfePowell(var c1: Double = 0.1, //通常在(0,0.5)之间,指的是ρ
                    ...
                    var init_interval: (Double, Double) = (1e-3, 1000), //初始化搜索区间
  ...
  def zoom(alpha_lo: Double,
    ...
    breakable {
      while (loop > 0) {
        ...
        println(s"zoom loop $loop is $alpha in [$low, $high]")
        //防止区间过小
        if (abs(high - low) < eps) {
          break
        }
        ...
        interpolationMethod match {
          case Interpolation.TWO_POINT_QUADRATIC => alpha = low - 0.5 * (low - high) / (1 - (low_loss - high_loss) / (low_grad.t * dk).data(0) * (low - high))
        ...
        if (f > f_zero + c1 * alpha * grad_zero || f >= low_loss) {
          ...
          val grad = (autoGrad.grad.t * dk).data(0)
  ...
  override def getStep(autoGrad: AutoGrad, dk: DenseMatrix[Double]): Double = {
    ...
    val grad_zero = (grad.t * dk).data(0) // 记录最初的g(0)

    val theta = autoGrad.theta

    breakable {
      while (loop > 0) {
        println(s"getStep loop $loop is $alpha in [$alpha1, $alpha2]")
        ...
        val grad_new = (autoGrad.grad.t * dk).data(0)
