...
class Dataset(var data: DenseMatrix[Double], var label: DenseMatrix[Double] = null) {
  ...
  def shuffle(inplace: Boolean = false): (DenseMatrix[Double], DenseMatrix[Double]) = {
    if(label != null) assert(data.rows == label.rows)
    ...
    val new_label = if(label == null) label else new DenseMatrix(label.size, 1, values_f(label))
  ...
  def get_minibatch(minibatch_size: Int): Seq[(DenseMatrix[Double], DenseMatrix[Double])] = {

    if (minibatch_size >= data.rows) Seq((data, label))
    else {
      val (new_data, new_label) = shuffle()
